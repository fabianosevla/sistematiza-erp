...
export async function PUT(req: NextRequest, { params }: Params) {
  try {
    ...
    try {
      ...
      if ('comandasAtivo'    in body) ops.push(db.execute(sql`UPDATE t_configuracoes_tenant SET comandas_ativo    = ${body.comandasAtivo},    updated_dt = ${now} WHERE active_flg = true`))
      if ('producaoAtivo'    in body) ops.push(db.execute(sql`UPDATE t_configuracoes_tenant SET producao_ativo    = ${body.producaoAtivo},    updated_dt = ${now} WHERE active_flg = true`))
      if ('estoqueAtivo'     in body) ops.push(db.execute(sql`UPDATE t_configuracoes_tenant SET estoque_ativo     = ${body.estoqueAtivo},     updated_dt = ${now} WHERE active_flg = true`))
      if ('fiscalAtivo'      in body) ops.push(db.execute(sql`UPDATE t_configuracoes_tenant SET fiscal_ativo      = ${body.fiscalAtivo},      updated_dt = ${now} WHERE active_flg = true`))
      if ('consultasAtivo'   in body) ops.push(db.execute(sql`UPDATE t_configuracoes_tenant SET consultas_ativo   = ${body.consultasAtivo},   updated_dt = ${now} WHERE active_flg = true`))
      if ('pedidosAtivo'     in body) ops.push(db.execute(sql`UPDATE t_configuracoes_tenant SET pedidos_ativo     = ${body.pedidosAtivo},     updated_dt = ${now} WHERE active_flg = true`))
      if ('planoAcaoAtivo'   in body) ops.push(db.execute(sql`UPDATE t_configuracoes_tenant SET plano_acao_ativo  = ${body.planoAcaoAtivo},   updated_dt = ${now} WHERE active_flg = true`))
      if ('metasAtivo'       in body) ops.push(db.execute(sql`UPDATE t_configuracoes_tenant SET metas_ativo       = ${body.metasAtivo},       updated_dt = ${now} WHERE active_flg = true`))
