...
export async function GET(req: NextRequest, { params }: Params) {
  try {
    ...
    try {
      ...
      const receitaRes = await db.execute(sql`
      ...
      `)
      const despesaRes = await db.execute(sql`
    ...
    try {
      ...
      if (body.tipo === 'simular') {
        ...
        for (const item of itens) {
          ...
          `)
          const preco = Number(prodRes.rows[0]?.preco_varejo ?? 0)
          ...
          `)
          for (const fi of fichaRes.rows as any[]) {
        ...
        const despesasRes = await db.execute(sql`
        ...
        `)
        const gastosRes = await db.execute(sql`
          SELECT COALESCE(SUM(valor), 0)::bigint as total FROM t_gasto_fixo_valor
          WHERE active_flg = true AND mes = ${mesSim} AND ano = ${anoSim}
        `).catch(() => ({ rows: [{ total: 0 }] }))

        const despesasMesVal = Number(despesasRes.rows[0]?.total ?? 0)
        const gastosFixosVal = Number(gastosRes.rows[0]?.total ?? 0)
        ...
        if (metaRow && receitaProjetada < metaRow.metaReceita) {
          ...
          if (itens.length > 0) {
            ...
            if (prod) {
              const preco = Number(prod.preco_varejo ?? 1)
              const unidadesExtra = Math.ceil(deficit / preco)
              sugestoes.push(`Venda +${unidadesExtra} unidades de ${prod.nome} para atingir a meta de receita`)
        ...
        return ok({ receitaProjetada, custoInsumos: custoArredondado, lucroBruto, despesasMes: despesasMesVal, gastosFixos: gastosFixosVal, totalDespesas, lucroLiquido, meta: metaRow ?? null, sugestoes })
      }
