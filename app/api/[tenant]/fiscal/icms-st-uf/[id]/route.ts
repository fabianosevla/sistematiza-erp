// @ts-nocheck
// ESTE ARQUIVO VAI EM: app/api/[tenant]/fiscal/icms-st-uf/[id]/route.ts
...
export async function PUT(req: NextRequest, { params }: Params) {
  try {
    ...
    try {
      ...
      const r = await new IcmsStUfService(db).atualizar(Number(params.id), body, userId)
...
export async function DELETE(req: NextRequest, { params }: Params) {
  try {
    ...
    try {
      if (!(await fiscalLigado(db))) return forbidden()
      const userId = await usuarioAtualIdDb(db)
      return ok(await new IcmsStUfService(db).excluir(Number(params.id), userId))
    } finally { release() }
