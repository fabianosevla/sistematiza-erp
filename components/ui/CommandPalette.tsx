...
interface ResultItem { id: string; label: string; sub?: string; href: string; icon: any; categoria: string }
...
export default function CommandPalette({ tenantSlug, open, onClose }: Props) {
  ...
  const { data: produtosData } = useQuery({
    ...
    queryFn:  async () => query.length < 2 ? { data: { data: [] } } : (await fetch(`/api/${tenantSlug}/cadastros/produtos?busca=${encodeURIComponent(query)}&limit=4`)).json(),
    enabled: query.length >= 2,
  })

  const { data: clientesData } = useQuery({
    queryKey: ['cmd-clientes', tenantSlug, query],
    queryFn:  async () => query.length < 2 ? { data: { data: [] } } : (await fetch(`/api/${tenantSlug}/cadastros/clientes?busca=${encodeURIComponent(query)}&limit=4`)).json(),
    enabled: query.length >= 2,
  })

  const rotasFiltradas = query ? rotas.filter(r => r.label.toLowerCase().includes(query.toLowerCase())) : rotas.slice(0, 6)

  const prodFiltrados: ResultItem[] = (Array.isArray(produtosData?.data?.data) ? produtosData.data.data : Array.isArray(produtosData?.data) ? produtosData.data : [])
    .map((p: any) => ({ id: `p_${p.produtoId}`, label: p.nome, sub: p.tipo ?? 'Produto', href: `/${tenantSlug}/cadastros/produtos`, icon: Package, categoria: 'Produtos' }))

  const clientesFiltrados: ResultItem[] = (Array.isArray(clientesData?.data?.data) ? clientesData.data.data : Array.isArray(clientesData?.data) ? clientesData.data : [])
    .map((c: any) => ({ id: `c_${c.clienteId}`, label: c.nomeCompleto, sub: c.documento ?? 'Cliente', href: `/${tenantSlug}/cadastros/clientes`, icon: User, categoria: 'Clientes' }))

  const todosResultados = [...rotasFiltradas, ...prodFiltrados, ...clientesFiltrados]

  function navegar(item: ResultItem) { router.current.push(item.href); onClose() }
          ...
          {todosResultados.length === 0 ? (
            ...
            <div key={cat}>
              ...
              {itens.map(item => {
                ...
                return (
                  <button key={item.id} onClick={() => navegar(item)} onMouseEnter={() => setSelected(idx)}
