...
interface Props {
  tenantSlug: string; tenantName: string
  comandasAtivo: boolean; producaoAtivo: boolean; vendasAtivo: boolean; estoqueAtivo: boolean; fiscalAtivo: boolean
}
...
export default function Sidebar({ tenantSlug, tenantName, comandasAtivo, producaoAtivo, vendasAtivo, estoqueAtivo, fiscalAtivo }: Props) {
  ...
  const items = [
    ...
    { label: 'Plano de Ação',href: '/plano-acao',  icon: ClipboardCheck },
  ...
  const linkStyle = (active: boolean) => active ? { backgroundColor: 'rgba(46,204,113,0.12)', borderLeft: '2px solid #2ecc71' } : {}
    ...
    <aside className="flex flex-col w-60 min-h-screen flex-shrink-0" style={{ backgroundColor: 'var(--color-sidebar)' }}>
      ...
      <nav className="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto">
        {items.map(item => {
            ...
            return (
              <div key={item.label}>
                <button onClick={() => toggleGroup(item.label)}
                  className={cn('w-full flex items-center justify-between gap-3 px-3 py-2 rounded-lg text-sm transition-colors', anyActive ? 'text-white' : 'text-white/50 hover:text-white/80')}>
                ...
                {isOpen && (
                  <div className="ml-6 mt-0.5 space-y-0.5">
                    {item.children.map(child => {
                      ...
                      return (
                        <Link key={child.href} href={`${base}${child.href}`} style={linkStyle(active)}
                          className={cn('block px-3 py-1.5 rounded-md text-sm transition-colors', active ? 'text-white font-medium' : 'text-white/40 hover:text-white/70')}>
                          {child.label}
                        </Link>
                      )
                    })}
          ...
          return (
            <Link key={item.label} href={`${base}${(item as any).href ?? ''}`} style={linkStyle(active)}
              className={cn('flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-colors', active ? 'text-white font-medium' : 'text-white/50 hover:text-white/80')}>
