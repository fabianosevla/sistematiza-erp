...
// Item do menu vertical do painel de Configurações: seta que expande/recolhe
// o conteúdo, com o Salvar da própria seção no rodapé — em vez de aba
// horizontal dividindo um Salvar só entre vários blocos sem relação.
//
// Fica fora do Header (não dentro do corpo da função) de propósito: um
// componente definido dentro de outro componente ganha identidade nova a
// cada render, e o React desmonta e remonta o acordeão inteiro toda vez que
// o Header re-renderiza — perde o estado de foco do campo sendo digitado.
...
export default function Header({
  ...
  // Cada seção do painel abre/fecha por conta própria — não é mais aba
  // exclusiva, é acordeão: pode ter mais de uma aberta ao mesmo tempo.
  ...
  return (
    <>
      ...
      {/* Painel de configurações: menu vertical em acordeão, cada seção com
          o próprio Salvar. Nada aqui grava sozinho. */}
      {showSettings && (
        <SidePanel
        ...
        >
            {/* Menu vertical em acordeão — cada item é uma seção com a própria
                seta e o próprio Salvar, em vez de abas horizontais dividindo
                um Salvar só. Seção sem formulário (Meu perfil, Arquivos) não
                tem botão: Meu perfil já grava no Clerk assim que a foto é
                escolhida, e Arquivos é só um link de download. */}

            {/* ── CAIXA ──────────────────────────────────────────────────
                Só aparece com o controle de caixa contratado. Com um PC só,
                o sistema assume o caixa 1 e nunca pergunta qual é —
            ...
            {/* ── ARQUIVOS ───────────────────────────────────────────────
                ...
                única exceção é o parâmetro --kiosk-printing, que só existe
                como argumento de atalho do sistema — e atalho é coisa que
                página web não cria, mas arquivo baixado cria.
            ...
            {/* ── MEU PERFIL ─────────────────────────────────────────────
                ...
                avatar por usuário e serve a imagem por CDN. Assim não
                precisou de coluna nova, rota nova nem migration — e a foto
                aparece no rodapé do menu na hora, sem precisar de Salvar. */}
