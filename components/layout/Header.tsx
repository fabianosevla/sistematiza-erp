...
import { Settings, LogOut, X, ToggleLeft, ToggleRight, Menu, Search, Bell, AlertTriangle, Info, Moon, Sun } from 'lucide-react'
...
interface Props {
  ...
  onMenuToggle: () => void; onPaletteOpen: () => void
    ...
    <>
      <header className="h-14 bg-white border-b border-gray-100 flex items-center justify-between px-4 lg:px-6 flex-shrink-0">
        <div className="flex items-center gap-3">
          <button onClick={onMenuToggle} className="lg:hidden p-1.5 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors"><Menu size={20} /></button>
          <nav className="flex items-center gap-1.5 text-sm">
            <Link href={`/${tenantSlug}`} className="text-gray-400 hover:text-gray-700 font-medium transition-colors">Início</Link>
            {crumbs.map((crumb, i) => (
              <span key={i} className="flex items-center gap-1.5">
                <span className="text-gray-300 text-xs">/</span>
                {crumb.isLast ? <span className="text-gray-900 font-semibold">{crumb.label}</span> : <Link href={crumb.href} className="text-gray-400 hover:text-gray-700 font-medium transition-colors">{crumb.label}</Link>}
              </span>
            ))}
          </nav>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button onClick={onPaletteOpen} className="hidden sm:flex items-center gap-2 px-3 py-1.5 text-sm text-gray-400 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors">
            <Search size={14} /><span className="text-xs">Buscar</span><kbd className="ml-1 px-1.5 py-0.5 text-[10px] bg-white rounded border border-gray-200">⌘K</kbd>
          </button>

          <button onClick={onToggleDark} className="p-2 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors" title={darkMode ? 'Modo Claro' : 'Modo Escuro'}>
            {darkMode ? <Sun size={18} /> : <Moon size={18} />}
          </button>

          <div className="relative">
            <button onClick={() => { setShowNotifs(!showNotifs); setShowMenu(false) }} className="relative p-2 text-gray-500 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors">
              <Bell size={18} />
              {cntAlerts > 0 && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center">{cntAlerts > 9 ? '9+' : cntAlerts}</span>}
            </button>
            {showNotifs && (
              <>
                ...
                <div className="absolute right-0 top-10 z-20 w-80 bg-white rounded-xl shadow-xl border border-gray-100 py-1 overflow-hidden">
                  <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                    ...
                    {notifs.length > 0 && <span className="text-xs bg-red-100 text-red-600 px-2 py-0.5 rounded-full font-medium">{notifs.length}</span>}
                  </div>
                  {notifs.length === 0 ? (
                    <div className="px-4 py-6 text-center"><Bell size={20} className="text-gray-200 mx-auto mb-2" /><p className="text-sm text-gray-400">Nenhuma notificação</p></div>
                  ) : (
                    <div className="max-h-72 overflow-y-auto divide-y divide-gray-50">
                      {notifs.map((n: any) => (
                        <Link key={n.id} href={`/${tenantSlug}/${n.href}`} onClick={() => setShowNotifs(false)} className="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                          <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${n.nivel === 'warning' ? 'bg-amber-100' : 'bg-blue-100'}`}>
                            {n.nivel === 'warning' ? <AlertTriangle size={13} className="text-amber-600" /> : <Info size={13} className="text-blue-600" />}
                          </div>
                          <div className="min-w-0"><p className="text-xs font-semibold text-gray-800">{n.titulo}</p><p className="text-xs text-gray-500 mt-0.5 truncate">{n.descricao}</p></div>
                        </Link>
                      ))}
                    </div>
                  )}
          ...
          <div className="relative">
            <button onClick={() => { setShowMenu(!showMenu); setShowNotifs(false) }} className="w-8 h-8 rounded-full bg-gray-900 flex items-center justify-center text-white text-sm font-medium hover:bg-gray-700 transition-colors">
            ...
            {showMenu && (
              <>
                ...
                <div className="absolute right-0 top-10 z-20 w-52 bg-white rounded-xl shadow-lg border border-gray-100 py-1">
                  ...
                  <button onClick={() => { setShowMenu(false); setShowSettings(true) }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-gray-50"><Settings size={14} /> Configurações</button>
                  <button onClick={() => signOut()} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50"><LogOut size={14} /> Sair</button>
                </div>
              </>
            )}
          </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="flex items-center justify-between py-2.5">
                  <div><p className="text-sm font-medium text-gray-900">Modo Escuro</p><p className="text-xs text-gray-400 mt-0.5">Alterna entre tema claro e escuro</p></div>
                  <button onClick={onToggleDark}>{darkMode ? <ToggleRight size={32} className="text-green-500" /> : <ToggleLeft size={32} className="text-gray-300" />}</button>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {[
                    { label: 'Nome da empresa', key: 'nomeEmpresa' }, { label: 'CNPJ', key: 'cnpj' },
                    { label: 'IE Estadual', key: 'ieEstadual' }, { label: 'UF', key: 'uf' },
                    { label: 'Regime Tributário (1=SN,3=LR/LP)', key: 'regimeTributario' },
                    { label: 'Telefone', key: 'telefone' }, { label: 'Endereço', key: 'endereco' },
                  ].map(f => (
              ...
              {config?.fiscalAtivo && (
                <div>
                  ...
                  <div className="space-y-3">
                    <div><Label className="text-xs">Token Focus NFe</Label><Input defaultValue={config?.focusNfeToken ?? ''} onBlur={e => mut.mutate({ focusNfeToken: e.target.value })} className="mt-1 h-8 text-sm font-mono" /></div>
                    <div><Label className="text-xs">Ambiente</Label>
                      <select defaultValue={config?.focusNfeAmbiente ?? 'homologacao'} onChange={e => mut.mutate({ focusNfeAmbiente: e.target.value })} className="mt-1 w-full h-8 rounded-lg border border-gray-200 px-3 text-sm focus:outline-none">
