...
export default function ConsultasView({ tenantSlug }: Props) {
  ...
  const { data: porProdData, isLoading: ppLoading } = useQuery({
  ...
  const metaVendas = vendasData?.meta
  const porProd  = porProdData ?? []
      ...
      {aba === 'insumos' && (
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100">
                {['Insumo','Est. Atual','Est. Mínimo','Unidade','Tipo','Preço Custo'].map((h,i) => (
            ...
            <tbody>
              {insumosLoading ? (
                ...
                <tr key={ins.insumoId} className={`border-b border-gray-50 ${ins.estoqueAtual <= ins.estoqueMinimo ? 'bg-red-50/30' : ''}`}>
                  ...
                  <td className="px-4 py-2.5 text-right text-sm text-gray-500">{ins.tipo ?? '—'}</td>
            ...
            <table className="w-full min-w-max">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  ...
                  <th className="text-left text-xs font-medium text-gray-400 px-3 py-3 w-16">Tipo</th>
              ...
              <tbody>
                {produtosLoading ? (
                  ...
                  const diasSemana = Object.values(semana)
                  return (
                    <tr key={p.produtoId} className="border-b border-gray-50">
                      ...
                      <td className="px-3 py-2.5 text-sm text-gray-500">{p.tipo ?? '—'}</td>
                      {Array.from({length:6},(_,i) => i).map(i => {
                        const val = diasSemana[i] as number | undefined
                        return (
