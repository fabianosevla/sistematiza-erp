...
export default function ProdutosView({ tenantSlug }: Props) {
  ...
  const addFichaMut = useMutation({
    mutationFn: () => fetch(`${api}/${showFicha.produtoId}/ficha`, {
      ...
      body: JSON.stringify({ insumoId: Number(fichaInsumoId), quantidade: parseFloat(fichaQtd), unidade: fichaUnidade }),
    }).then(r => r.json()),
    onSuccess: () => { refetchFicha(); setFichaInsumoId(''); setFichaQtd(''); toast('Insumo adicionado!') },
  })

  const removeFichaMut = useMutation({
    mutationFn: (id: number) => fetch(`${api}/${showFicha.produtoId}/ficha/${id}`, { method: 'DELETE' }).then(r => r.json()),
  ...
  function exportCSV() {
    const rows = todos.map((p: any) => [p.produtoId, p.nome, p.tipo ?? '', p.unidade ?? '', p.precoVarejo ? (p.precoVarejo/100).toFixed(2) : '0', p.estoqueAtual, p.estoqueMinimo, p.activeFlag ? 'Ativo' : 'Inativo'])
    const csv  = [['ID','Nome','Tipo','Unidade','Preço Varejo','Est.Atual','Est.Mín','Status'], ...rows].map(r => r.map((c: any) => `"${c}"`).join(',')).join('\n')
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setShowInativos(v => !v)}
            ...
            <EyeOff size={14} className="mr-1.5" /> {showInativos ? 'Ocultar inativos' : 'Ver inativos'}
          </Button>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-100">
              ...
              <th className="text-center text-xs font-medium text-gray-400 px-4 py-3 cursor-pointer select-none hover:text-gray-600" onClick={() => toggleSort('precoVarejo')}>Varejo <SortIcon col="precoVarejo" /></th>
            ...
            {isLoading ? (
              ...
              return (
                <tr key={p.produtoId} className={`group border-b border-gray-50 transition-colors ${inativo ? 'opacity-50 bg-gray-50/50' : 'hover:bg-gray-50/80'}`}>
                  ...
                  <td className="px-4 py-3 text-center text-sm text-gray-500">{(p.precoAtacadoA ?? p.precoAtacado) ? fmt(p.precoAtacadoA ?? p.precoAtacado) : '—'}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      {inativo ? (
                        ...
                        <button onClick={() => setConfirmDelete({ id: p.produtoId, nome: p.nome })} className="p-1 text-gray-300 hover:text-red-500"><Trash2 size={14} /></button>
                      )}
                    </div>
                  </td>
                </tr>
              ...
              <div>
                ...
                <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                  <div>
                    ...
                    <Input type="number" min="0" step="0.01" value={precoVarejo} onChange={e => setPrecoVarejo(e.target.value)} className="mt-1 h-9" placeholder="0,00" />
                  </div>
            ...
            <div className="p-6">
              <div className="bg-gray-50 rounded-lg p-4 mb-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div><Label className="text-xs">Quantidade *</Label><Input type="number" min="0" step="0.001" value={fichaQtd} onChange={e => setFichaQtd(e.target.value)} className="mt-1 h-9 text-sm" /></div>
                  <div>
                    <Label className="text-xs">Unidade</Label>
                    <select value={fichaUnidade} onChange={e => setFichaUnidade(e.target.value)} className="mt-1 w-full h-9 rounded-lg border border-gray-200 px-3 text-sm focus:outline-none">
                      {unidades.map(u => <option key={u} value={u}>{u}</option>)}
                    </select>
                  </div>
                </div>
                <Button size="sm" className="mt-3" onClick={() => addFichaMut.mutate()} disabled={!fichaInsumoId || !fichaQtd || addFichaMut.isPending}>
              ...
              {fichaItens.length === 0 ? (
                ...
                <table className="w-full">
                  <thead><tr className="border-b border-gray-100">{['Insumo','Qtd / unidade','Unidade',''].map((h,i) => <th key={i} className={`text-${i===0?'left':'center'} text-xs font-medium text-gray-400 px-3 py-2`}>{h}</th>)}</tr></thead>
                  <tbody>
                    {fichaItens.map((item: any) => (
                      <tr key={item.produtoInsumoId ?? item.itemId} className="border-b border-gray-50 group">
                        <td className="px-3 py-2.5 text-sm font-medium text-gray-900">{item.nomeInsumo ?? `#${item.insumoId}`}</td>
                        ...
                        <td className="px-3 py-2.5 text-center">
                          <button onClick={() => removeFichaMut.mutate(item.produtoInsumoId ?? item.itemId)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={13} /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
