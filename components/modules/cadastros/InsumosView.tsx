...
export default function InsumosView({ tenantSlug }: Props) {
  ...
  function abrirModal(item?: any) {
    if (item) {
      ...
      setEditando(null)
      setNome(''); setTipo(tipos[0] ?? ''); setUnidade(unidades[0] ?? '')
  ...
  function exportCSV() {
    const rows = todosInsumos.map((i: any) => [
      i.insumoId, i.nome, i.tipo ?? '', i.unidade ?? '',
      i.estoqueAtual, i.estoqueMinimo,
      i.precoCusto ? (i.precoCusto / 100).toFixed(2) : '0',
    ])
    const csv = [
      ['ID', 'Nome', 'Tipo', 'Unidade', 'Estoque Atual', 'Estoque Mínimo', 'Preço Custo'],
      ...rows,
  ...
  const todosInsumos = Array.isArray(raw?.data?.data) ? raw.data.data
  ...
  return (
    <div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-100">
              ...
              <th className="text-center text-xs font-medium text-gray-400 px-4 py-3 cursor-pointer select-none hover:text-gray-600"
                onClick={() => toggleSort('estoqueAtual')}>Est. Atual <SortIcon col="estoqueAtual" /></th>
              <th className="text-center text-xs font-medium text-gray-400 px-4 py-3">Est. Mínimo</th>
              <th className="text-center text-xs font-medium text-gray-400 px-4 py-3 cursor-pointer select-none hover:text-gray-600"
                onClick={() => toggleSort('precoCusto')}>Preço Custo <SortIcon col="precoCusto" /></th>
              <th className="w-16" />
            </tr>
          </thead>
          <tbody>
            {isLoading ? (
              ...
              <tr>
                <td colSpan={7}>
                  <EmptyState
                    ...
                    action="Cadastrar primeiro insumo"
                    onAction={() => abrirModal()}
                  />
                </td>
              </tr>
              ...
              <tr key={ins.insumoId}
                className={`group border-b border-gray-50 hover:bg-gray-50/80 transition-colors ${ins.estoqueAtual <= ins.estoqueMinimo ? 'bg-red-50/20' : ''}`}>
                ...
                <td className="px-4 py-3 text-center text-sm font-medium text-gray-700">
                  {ins.precoCusto ? fmt(ins.precoCusto) : '—'}
                </td>
                <td className="px-4 py-3 text-center">
                  <button
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <Input type="number" min="0" step="0.01" value={precoCusto}
                    onChange={e => setPrecoCusto(e.target.value)} className="mt-1" />
                </div>
              </div>
      ...
      {showImport && (
        <CsvImportModal
          tenantSlug={tenantSlug} entidade="insumos" nomeEntidade="Insumos"
