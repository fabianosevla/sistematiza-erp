...
export default function Ficha360View({ tenantSlug, clienteId }: Props) {
  ...
  const colunasPedidos: Coluna[] = [
    ...
    { chave: 'status', titulo: 'Status', render: (p: any) => <Badge variant="outline">{p.status}</Badge> },
  ]
    ...
    <div>
      ...
      <div className="space-y-6">
        <div>
          ...
          <DataTable colunas={colunasVendas} itens={vendas} chave={(v: any) => v.vendaId} vazio="Nenhuma venda ainda." />
        </div>

        {pedidos.length > 0 && (
          <div>
            <p className="text-sm font-semibold text-gray-700 mb-2 inline-flex items-center gap-1"><Package size={14} /> Pedidos</p>
            <DataTable colunas={colunasPedidos} itens={pedidos} chave={(p: any) => p.pedidoId} vazio="Nenhum pedido ainda." />
          </div>
        )}
